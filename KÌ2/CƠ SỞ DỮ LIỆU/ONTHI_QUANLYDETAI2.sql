...
('D02',N'CÔNG NGHỆ BLOCKEHAIN','2018','GV02'),
...
-----CAU4
SELECT GIAOVIEN.MAGV,HOTEN,COUNT(DETAI.MADT) SODTHUONGDAN FROM GIAOVIEN 
JOIN DETAI ON GIAOVIEN.MAGV = DETAI.MAGV
GROUP BY  GIAOVIEN.MAGV, HOTEN
HAVING COUNT( DETAI.MADT) >= ALL(SELECT COUNT(MADT) FROM DETAI GROUP BY MAGV)

-----CAU4
SELECT GIAOVIEN.MAGV, HOTEN,COUNT(MADT) FROM GIAOVIEN
 JOIN DETAI ON GIAOVIEN.MAGV = DETAI.MAGV
 GROUP BY GIAOVIEN.MAGV, HOTEN
 HAVING COUNT(MADT) >= ALL(SELECT COUNT(MADT) FROM DETAI GROUP BY MAGV);


 ----CAU5
 SELECT DETAI.MADT,TENDT, AVG(DIEN) DIEMTB FROM SINHVIEN 
 JOIN DETAI ON SINHVIEN.MADT = DETAI.MADT
 GROUP BY DETAI.MADT,TENDT


---CAU5
SELECT SINHVIEN.MADT,TENDT,AVG(DIEN) DIEMTB FROM SINHVIEN 
JOIN DETAI ON SINHVIEN.MADT = DETAI.MADT
GROUP BY  SINHVIEN.MADT,TENDT
----CAU7
UPDATE GIAOVIEN
