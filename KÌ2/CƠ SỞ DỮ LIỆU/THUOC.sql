...
WHERE GIA = GIA * 0.9 OR NHOMDIEUTRI = N'COVIT19' 
...
---CAU1
SELECT THUOC.MATHUOC,TENTHUOC,SOLUONG FROM THUOC 
JOIN TOATHUOC ON THUOC.MATHUOC = TOATHUOC.MATHUOC
JOIN KHAMBENH ON KHAMBENH.MAKB = TOATHUOC.MAKB
JOIN HOSO ON HOSO.MAHS = KHAMBENH.MAHS
WHERE HOSO.MAHS = '024'
--CAU2
SELECT HOSO.MAHS,HOTEN,BACSI,KETLUAN, YEAR(GETDATE())-YEAR(NGAYSINH) TUOI FROM HOSO 
JOIN KHAMBENH ON KHAMBENH.MAHS = HOSO.MAHS
WHERE HOSO.MAHS = '025'

---CAU3
SELECT HOSO.MAHS,HOTEN,BACSI,KETLUAN,NGAYKHAM FROM HOSO 
JOIN KHAMBENH ON KHAMBENH.MAHS = HOSO.MAHS
WHERE MAKB = 'MKB34'
...
---CAU4
SELECT NHOMDIEUTRI, GIA FROM THUOC
WHERE GIA > 500 OR GIA < 1000


--CAU5
SELECT NGAYKHAM,BACSI,KETLUAN ,NGAYSINH FROM KHAMBENH JOIN HOSO ON HOSO.MAHS = KHAMBENH.MAHS
WHERE YEAR(NGAYSINH) = '1975'

--CAU6
SELECT NGAYKHAM,BACSI,KETLUAN FROM KHAMBENH 
JOIN HOSO ON HOSO.MAHS = KHAMBENH.MAHS
WHERE HOTEN = N'NGUYỄN VĂN HUY'
GROUP BY NGAYKHAM,BACSI,KETLUAN 
--CAU7
